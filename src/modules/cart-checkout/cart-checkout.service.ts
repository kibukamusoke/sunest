...
export class CartCheckoutService {
  ...
  async getUserCart(
    ...
    // If user is authenticated, get/create user cart
    if (userId) {
    ...
    // For guest users, get/create cart by sessionId
    if (sessionId) {
      let cart = await this.prisma.cart.findFirst({
        where: {
          sessionId,
      ...
      if (!cart) {
        ...
        cart = await this.prisma.cart.create({
          data: {
            sessionId,
            isGuest: true,
            expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days for guest carts
          },
          include: this.getCartIncludeOptions(),
        });
      }
  ...
  async mergeGuestCartOnLogin(
    ...
    // If guest cart exists and has items, merge them
    if (guestCart && guestCart.items.length > 0) {
      this.logger.log(
        `Merging guest cart ${guestCart.id} with user cart ${userCart.id} for user ${userId}`,
      );
      ...
      // Deactivate guest cart after successful merge
      await this.prisma.cart.update({
  ...
  async bulkAddToCart(
    ...
    for (const item of bulkAddDto.items) {
      try {
        const cartItem = await this.addItemToCart(
          cartId,
          {
            ...item,
  ...
  async addQuoteToCart(
    ...
    const itemsToAdd = quoteToCartDto.quoteItemIds
      ? quote.items.filter((item) =>
          quoteToCartDto.quoteItemIds!.includes(item.id),
        )
      : quote.items;
    ...
    for (const quoteItem of itemsToAdd) {
      try {
        ...
        this.logger.warn(
          `Failed to add quote item ${quoteItem.id} to cart: ${error.message}`,
        );
      }
    }
    ...
    for (const item of cart.items) {
      ...
      if (item.product) {
        const inventoryItems =
          item.productVariant?.inventoryItems || item.product.inventoryItems;
        ...
        if (totalAvailable < item.quantity) {
          ...
          itemValidation.errors.push(
            `Insufficient stock: ${totalAvailable} available, ${item.quantity} requested`,
          );
  ...
  async getAddresses(
    ...
    // For B2B, addresses belong to company, not individual users
    ...
    const addressDtos = addresses.map((address) =>
      this.mapAddressToResponseDto(address),
    );

    return {
      addresses: addressDtos,
      total: addressDtos.length,
      defaultShipping: addressDtos.find(
        (addr) =>
          addr.isDefault &&
          (addr.type === AddressType.SHIPPING ||
            addr.type === AddressType.BOTH),
      ),
      defaultBilling: addressDtos.find(
        (addr) =>
          addr.isDefault &&
  ...
  private calculateCartSummary(items: any[]) {
    const activeItems = items.filter(
      (item) => item.status === CartItemStatus.ACTIVE,
    );
    ...
    return {
      ...
      itemsRequiringApproval: activeItems.filter(
        (item) =>
          !item.isApproved && parseFloat(item.totalPrice.toString()) > 1000,
      ).length,
      unavailableItems: activeItems.filter(
        (item) => !this.calculateItemAvailability(item).isAvailable,
      ).length,
    };
  }
