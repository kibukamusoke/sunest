...
export class RFQManagementService {
  ...
  async listRFQs(
    ...
    const whereClause = await this.buildRFQWhereClause(
      filterDto,
      userId,
      userRoles,
    );
  ...
  async createQuote(
    ...
    if (existingQuote) {
      throw new BadRequestException(
        'A quote for this RFQ already exists. Use update or create a revision.',
      );
    }
  ...
  async acceptQuote(
    ...
    if (quote.rfq.requesterId !== userId) {
      throw new ForbiddenException(
        'Only the RFQ requester can accept this quote',
      );
    }
  ...
  async rejectQuote(
    ...
    if (quote.rfq.requesterId !== userId) {
      throw new ForbiddenException(
        'Only the RFQ requester can reject this quote',
      );
    }
    ...
    const rejectedQuote = await this.prisma.quote.update({
      ...
      data: {
        ...
        customerNotes:
          `${quote.customerNotes || ''}\n\nRejection Reason: ${rejectDto.rejectionReason}${
    ...
    await this.notifyMerchantOfQuoteRejection(
      rejectedQuote,
  ...
  async listQuotes(
    ...
    // Verify RFQ exists if rfqId is provided
    if (filterDto.rfqId) {
      const rfq = await this.prisma.rFQ.findUnique({
      ...
      // Check access permissions for the RFQ
      await this.checkRFQAccess(rfq, userId);
    }

    // Build where clause based on filters and user role
    const whereClause = await this.buildQuoteWhereClause(
      filterDto,
      userId,
      userRoles,
    );
  ...
  async addRFQComment(
    rfqId: string,
    commentDto: CreateRFQCommentDto,
    userId: string,
  ...
  async getRFQComments(
    ...
    return {
      comments: comments.map((comment) =>
        this.mapToRFQCommentResponse(comment),
      ),
      total,
      publicCount,
  ...
  private async buildRFQWhereClause(
    filterDto: RFQFilterDto,
    userId: string,
    userRoles: string[],
  ): Promise<Prisma.RFQWhereInput> {
  ...
  private async buildQuoteWhereClause(
    filterDto: QuoteFilterDto,
    userId: string,
    userRoles: string[],
    ...
    // Apply role-based filtering
    if (userRoles.includes('buyer')) {
      // Buyers can only see quotes for their RFQs
      whereClause.rfq = {
        requesterId: userId,
      };
    } else if (
      ...
      // Merchants can only see their own quotes
      // Get merchant IDs from user's merchants
      ...
      if (userMerchants.length > 0) {
        ...
        // If user has no merchants, return empty result
        whereClause.merchantId = '00000000-0000-0000-0000-000000000000';
      }
    }
    // system_admin can see all quotes (no additional filter)
    ...
    if (filterDto.status) {
      whereClause.status = filterDto.status as QuoteStatus;
    }
    ...
    if (filterDto.minAmount || filterDto.maxAmount) {
      ...
      if (filterDto.minAmount) {
        whereClause.totalAmount.gte = new Decimal(filterDto.minAmount);
      }
      if (filterDto.maxAmount) {
        whereClause.totalAmount.lte = new Decimal(filterDto.maxAmount);
      }
    }
  ...
  private async notifyRequesterOfNewQuote(quote: any): Promise<void> {
    ...
    console.log(
      `New quote ${quote.quoteNumber} received for RFQ ${quote.rfq.rfqNumber}`,
    );
  }
  ...
  private async notifyMerchantOfQuoteRejection(
    quote: any,
    reason: string,
  ): Promise<void> {
