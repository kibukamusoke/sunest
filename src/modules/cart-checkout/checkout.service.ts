...
import { CreateOrderDto } from '../order-fulfillment/dto/order.dto';
import {
  Prisma,
  CheckoutStatus,
  ApprovalStatus,
...
export class CheckoutService {
  ...
  async initiateCheckout(
    initiateDto: InitiateCheckoutDto,
    userId: string,
    ...
    // Get default company addresses (B2B - addresses belong to company, not individual users)
    // These are optional during initiation - user can select addresses during checkout
    ...
    // Calculate initial pricing (use default addresses if available, otherwise estimate without addresses)
    ...
    const approvalRequirement = await this.checkApprovalRequirement(
      userId,
      calculation.totalAmount,
      initiateDto.companyId,
    );
  ...
  async updateCheckoutShipping(
    ...
    // Verify address belongs to company (B2B - company addresses, not user addresses)
  ...
  async submitCheckout(
    ...
    // Validate required addresses before submission
    if (!checkout.shippingAddressId || !checkout.billingAddressId) {
      throw new BadRequestException(
        'Shipping and billing addresses are required to complete checkout',
      );
    }
    ...
    if (!checkout.paymentMethod) {
      throw new BadRequestException(
        'Payment method is required to complete checkout',
      );
    }
    const requiresApproval =
      checkout.requiresApproval && !submitDto.bypassApproval;

    const statusUpdate: Prisma.CheckoutUpdateInput = {
      customerNotes: submitDto.customerNotes || checkout.customerNotes,
    };

    if (requiresApproval) {
      statusUpdate.status = CheckoutStatus.PENDING_APPROVAL;
      statusUpdate.approvalStatus = ApprovalStatus.PENDING;
      statusUpdate.approvalRequestedAt = new Date();
      statusUpdate.approvalRequestedBy = userId;
    } else {
      statusUpdate.status = CheckoutStatus.APPROVED;
      statusUpdate.approvalStatus = ApprovalStatus.APPROVED;
    ...
    if (requiresApproval) {
      const checkoutWithTotals = await this.updateCheckoutTotals(
        checkoutId,
        calculation,
      );
    ...
    const completedCheckout = await this.completeApprovedCheckout(
      updatedCheckout,
      calculation,
    );
  ...
  async approveCheckout(
    ...
    await this.sendApprovalDecisionNotification(
      updatedCheckout,
    ...
    if (approvalDto.approved) {
      ...
      this.logger.log(
        `Checkout approved and completed: ${checkoutId} by ${userId}`,
      );
      return completedCheckout;
    }
  ...
  async completeCheckout(
    ...
    if (checkoutRecord.status !== CheckoutStatus.APPROVED) {
      throw new BadRequestException(
        'Checkout must be approved before completion',
      );
    }

    const calculation = this.calculateCheckoutPricing(
      checkoutRecord.cart.items,
      checkoutRecord.shippingAddress,
      checkoutRecord.billingAddress,
    );
  ...
  async listCheckouts(
    ...
    if (!userRoles.includes('SystemAdmin')) {
      if (
        userRoles.includes('MerchantAdmin') ||
  ...
  private getUserApprovalLimit(user: any): number {
    ...
    if (!user.roles || user.roles.length === 0) {
      return 500; // Default for users with no roles
    }

    const roleNames = user.roles.map((r: any) => r.name);

    if (roleNames.includes('SystemAdmin')) return 1000000;
    if (roleNames.includes('MerchantAdmin')) return 50000;
    if (roleNames.includes('MerchantUser')) return 10000;
    if (roleNames.includes('Buyer')) return 1000;
    return 500; // Default
  }
  ...
  private async sendApprovalNotification(checkout: any): Promise<void> {
    try {
      ...
      const approverEmails = eligibleApprovers.map(
  ...
  private async updateCheckoutTotals(
    checkoutId: string,
  ...
  private buildOrderDtoFromCheckout(checkout: any): CreateOrderDto {
    if (!checkout.shippingAddressId || !checkout.billingAddressId) {
      throw new BadRequestException(
        'Shipping and billing addresses are required to create an order',
      );
    }
    ...
    const orderItems = checkout.cart.items.map((item: any) => ({
      ...
      unitPrice:
        item.unitPrice != null ? parseFloat(item.unitPrice.toString()) : 0,
  ...
  private async completeApprovedCheckout(
    checkout: any,
    calculation: CheckoutCalculationDto,
  ): Promise<CheckoutResponseDto> {
    const checkoutWithTotals = await this.updateCheckoutTotals(
    ...
    try {
      ...
      order = await this.orderFulfillmentService.createOrder(
        orderDto,
        checkoutWithTotals.userId,
      );
    } catch (error) {
      this.logger.error(
        `Failed to create order for checkout ${checkout.id}:`,
        error,
      );
      throw new BadRequestException('Failed to create order from checkout');
    }
  ...
  private async validateCheckoutAccess(checkoutId: string, userId: string) {
    ...
    if (
      ...
      throw new BadRequestException(
        'Cannot modify completed or cancelled checkout',
      );
    }
