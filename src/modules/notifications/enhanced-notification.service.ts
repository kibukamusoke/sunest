...
export class EnhancedNotificationService {
  ...
  async sendNotification(notificationId: string): Promise<boolean> {
    try {
      ...
      if (notification.status !== NotificationStatus.PENDING) {
        this.logger.warn(
          `Notification ${notificationId} is not in pending status`,
        );
        return false;
      }
  ...
  async getEmailQueue(
    query: EmailQueueListQueryDto,
  ): Promise<EmailQueueListResponseDto> {
    const page = query.page || 1;
    const limit = query.limit || 20;
    const skip = (page - 1) * limit;
  ...
  async getEmailStats(): Promise<EmailStatsDto> {
    const [totalSent, delivered, opened, clicked, bounced, failed] =
      await Promise.all([
  ...
  private async processEventRules(event: any): Promise<void> {
    ...
    for (const rule of rules) {
      ...
      if (
        rule.conditions &&
        !this.evaluateConditions(rule.conditions as any, event)
      ) {
        continue;
      }
