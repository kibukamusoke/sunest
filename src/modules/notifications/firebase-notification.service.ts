...
export class FirebaseNotificationService implements OnModuleInit {
  ...
  onModuleInit() {
    try {
      ...
      if (process.env.NODE_ENV === 'development' && privateKey === 'your-private-key') {
   ...
   */
  async sendToDevice(token: string, payload: admin.messaging.MessagingPayload): Promise<boolean> {
    if (!this.initialized) {
   ...
   */
  async sendToDevices(tokens: string[], payload: admin.messaging.MessagingPayload): Promise<boolean> {
    if (!this.initialized) {
      this.logger.warn('Firebase not initialized. Cannot send notification.');
      return false;
    }
   ...
   */
  async sendToTopic(topic: string, payload: admin.messaging.MessagingPayload): Promise<boolean> {
    if (!this.initialized) {
      this.logger.warn('Firebase not initialized. Cannot send notification.');
      return false;
    }
  ...
  async subscribeToTopic(token: string, topic: string): Promise<boolean> {
    ...
    try {
      const response = await this.firebaseApp.messaging().subscribeToTopic([token], topic);
  ...
  async unsubscribeFromTopic(token: string, topic: string): Promise<boolean> {
    ...
    try {
      const response = await this.firebaseApp.messaging().unsubscribeFromTopic([token], topic);
