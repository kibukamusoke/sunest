...
export class MetricsService {
  ...
  async createDataPoint(
    data: CreateDataPointDto,
  ): Promise<DataPointResponseDto> {
    try {
      ...
      this.logger.error(
        `Failed to create data point: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async createBulkDataPoints(dataPoints: CreateDataPointDto[]): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to create bulk data points: ${error.message}`,
        error.stack,
      );
      throw error;
    }
    ...
    try {
      ...
      switch (metric.type) {
        ...
        case MetricType.FULFILLMENT_TIME:
          value = await this.calculateAverageFulfillmentTime(
            startDate,
            endDate,
          );
          break;
        default:
          this.logger.warn(
            `Metric type ${metric.type} calculation not implemented`,
          );
          return;
      }
      ...
      this.logger.error(
        `Failed to calculate metric ${metric.name}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  private async calculateSalesRevenue(
    startDate: Date,
    endDate: Date,
  ): Promise<number> {
  ...
  private async calculateOrderCount(
    startDate: Date,
    endDate: Date,
  ): Promise<number> {
  ...
  private async calculateUserRegistrations(
    startDate: Date,
    endDate: Date,
  ): Promise<number> {
  ...
  private async calculateSearchQueries(
    startDate: Date,
    endDate: Date,
  ): Promise<number> {
    return this.prisma.analyticsEvent.count({
  ...
  private async calculateCustomerLifetimeValue(
    ...
    const avgOrderFreq =
      uniqueCustomers.length > 0 ? totalOrders / uniqueCustomers.length : 0;
    return aov * avgOrderFreq * 12; // Annualized
  }

  private async calculateAverageFulfillmentTime(
    startDate: Date,
    endDate: Date,
  ): Promise<number> {
