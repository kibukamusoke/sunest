...
export class RFQManagementController {
  ...
  @ApiOperation({
    ...
    description:
      'Create a new Request for Quote with multiple items and specifications',
  })
  @ApiResponse({
  ...
  async createQuote(
    ...
    if (!merchantId) {
      throw new Error(
        'User must be associated with a merchant to create quotes',
      );
    }
  ...
  async submitQuote(
    ...
    if (!merchantId) {
      throw new Error(
        'User must be associated with a merchant to submit quotes',
      );
    }
  ...
  async getRFQComments(
    ...
    return this.rfqService.getRFQComments(
      rfqId,
      userId,
