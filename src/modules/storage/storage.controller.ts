...
export class StorageController {
  ...
  async confirmUpload(@Body() confirmUploadDto: ConfirmUploadDto) {
    return {
      success: await this.s3StorageService.confirmFileUpload(
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Get a permanent public URL for a file' })
  @ApiResponse({
    status: 200,
    description: 'Returns a permanent public URL for the file',
  })
  async getPublicUrl(@Param() params: FileIdParamDto) {
    return this.s3StorageService.getPublicUrl(params.fileId);
  }
