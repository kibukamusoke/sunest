...
-- This handles categories and products tables
...
SELECT old_id, new_id FROM category_id_mapping ORDER BY old_id;
...
-- Step 6: Update products table if it exists and has data
DO $$
BEGIN
...
-- Step 7: Replace old columns with new ones
-- Categories table
...
\echo 'Final category count:'
SELECT COUNT(*) as total_categories FROM categories;

\echo 'Categories with proper UUID format:'
SELECT COUNT(*) as uuid_categories FROM categories 
WHERE id::text ~ '^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$';
