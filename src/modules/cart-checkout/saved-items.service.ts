...
export class SavedItemsService {
  ...
  async listSavedItems(
    ...
    const savedItemDtos = savedItems.map((item) =>
      this.mapSavedItemToResponseDto(item),
    );
    ...
    if (existingCartItem) {
      ...
      cartItem = await this.prisma.cartItem.update({
        ...
        data: {
          ...
          totalPrice:
            parseFloat(existingCartItem.unitPrice.toString()) *
  ...
  async bulkAddSavedItemsToCart(
    ...
    for (let i = 0; i < savedItems.length; i++) {
      ...
      try {
        ...
        this.logger.warn(
          `Failed to add saved item ${savedItem.id} to cart: ${error.message}`,
        );
      }
    }
  ...
  private mapSavedItemToResponseDto(savedItem: any): SavedItemResponseDto {
    ...
    const savedPrice = savedItem.savedPrice
      ...
      : null;
    const priceChange =
      currentPrice && savedPrice ? currentPrice - savedPrice : 0;
    const priceChangePercentage =
      savedPrice && savedPrice > 0 ? (priceChange / savedPrice) * 100 : 0;
