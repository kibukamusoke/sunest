...
export class TelegramNotificationService implements OnModuleInit {
  ...
  onModuleInit() {
    try {
      ...
      if (!token) {
        this.logger.warn(
          'Telegram Bot token missing. Telegram notifications are disabled.',
        );
        return;
      }
      ...
      if (
        ...
        this.logger.warn(
          'Using placeholder Telegram credentials. Telegram notifications will not work correctly.',
        );
        return;
      }
  ...
  async sendMessage(message: string, chatId?: string): Promise<boolean> {
    ...
    try {
      ...
      this.logger.debug(
        `Message sent to chat ${targetChatId}: ${response.message_id}`,
      );
      return true;
    } catch (error) {
      this.logger.error(
  ...
  async sendPhoto(
    ...
    try {
      ...
      this.logger.debug(
        `Photo sent to chat ${targetChatId}: ${response.message_id}`,
      );
      return true;
    } catch (error) {
      this.logger.error(
  ...
  async sendDocument(
    ...
    try {
      ...
      this.logger.debug(
        `Document sent to chat ${targetChatId}: ${response.message_id}`,
      );
      return true;
    } catch (error) {
      this.logger.error(
  ...
  async sendLocation(
    ...
    try {
      ...
      const response = await this.bot.sendLocation(
        targetChatId,
        latitude,
        longitude,
      );
      ...
      this.logger.debug(
        `Location sent to chat ${targetChatId}: ${response.message_id}`,
      );
      return true;
    } catch (error) {
      this.logger.error(
        `Error sending Telegram location: ${error.message}`,
        error.stack,
      );
      return false;
    }
