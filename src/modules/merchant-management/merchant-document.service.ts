...
export class MerchantDocumentService {
  ...
  async uploadDocument(
    ...
    // compare file.userId with userId
  ...
  async updateDocumentStatus(
    ...
    const updatedDocument = await this.prisma.merchantDocument.update({
      ...
      data: {
        ...
        verifiedAt:
          updateStatusDto.status === DocumentStatus.VERIFIED
            ? new Date()
            : null,
        verifiedBy:
          updateStatusDto.status === DocumentStatus.VERIFIED ? userId : null,
      },
      include: {
        file: true,
        merchant: true,
  ...
  async deleteDocument(documentId: string, merchantId?: string): Promise<void> {
    ...
    if (
      document.merchant.status !== 'PENDING' &&
      document.merchant.status !== 'REJECTED'
    ) {
      throw new BadRequestException(
        'Documents cannot be deleted for applications in current status',
      );
    }
