...
export class SearchDiscoveryService {
  ...
  async searchProducts(
    ...
    const include = this.buildSearchIncludeClause(
      includeVariants,
      includeInventory,
      includeCategory,
      includeMerchant,
    ...
    try {
      ...
      const searchProducts = await Promise.all(
        products.map((product) =>
          this.mapToSearchProduct(product, quantity, warehouseId),
        ),
      );
  ...
  async bulkSkuSearch(
    bulkSearchDto: BulkSkuSearchDto,
    warehouseId?: string,
    ...
    await Promise.all(
      products.map(async (product) => {
        const searchProduct = await this.mapToSearchProduct(
          product,
          1,
          warehouseId,
        );
  ...
  async getSearchSuggestions(
    ...
    return {
      ...
      popular,
      total:
        products.length + categories.length + brands.length + popular.length,
      searchTerm: term,
    };
  }
  ...
  async getSavedSearches(userId: string): Promise<SavedSearchListDto> {
    ...
    return {
      ...
      alertEnabledCount: searches.filter(
        (s) => s.alertOnNewResults || s.alertOnPriceChange,
      ).length,
    };
  }
  ...
  async executeSavedSearch(
    executeDto: ExecuteSavedSearchDto,
    userId: string,
  ): Promise<ProductSearchResponseDto> {
  ...
  async trackSearchAnalytics(
    analyticsDto: CreateSearchAnalyticsDto,
    userId?: string,
  ): Promise<void> {
    try {
  ...
  async getSearchAnalytics(
    queryDto: SearchAnalyticsQueryDto,
  ...
  private async mapToSearchProduct(
    product: any,
    quantity: number = 1,
    warehouseId?: string,
  ...
  private async getZeroResultSearches(
    whereClause: any,
    limit: number,
  ): Promise<ZeroResultSearchDto[]> {
  ...
  private async getSearchPerformance(
    whereClause: any,
