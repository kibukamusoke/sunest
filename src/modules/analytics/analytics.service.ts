...
export class AnalyticsService {
  ...
  async createSession(
    data: CreateAnalyticsSessionDto,
    ...
    try {
      ...
      this.logger.error(
        `Failed to create analytics session: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async updateSession(
    sessionId: string,
    data: UpdateAnalyticsSessionDto,
  ): Promise<AnalyticsSessionResponseDto> {
    try {
      ...
      this.logger.error(
        `Failed to update analytics session: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getSession(
    sessionId: string,
  ): Promise<AnalyticsSessionResponseDto | null> {
  ...
  async createEvent(
    ...
    try {
      ...
      this.logger.error(
        `Failed to create analytics event: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async calculateBusinessMetrics(
    ...
    try {
      this.logger.log(
        `Calculating business metrics for ${startDate} to ${endDate}`,
      );
      ...
      this.logger.error(
        `Failed to calculate business metrics: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  private async calculateMetricsForDate(
    date: Date,
  ...
  async getAnalyticsOverview(
    ...
    try {
      ...
      const [previousRevenue, previousOrders, previousUsers, previousAOV] =
        await Promise.all([
      ...
      const ordersGrowth = this.calculateGrowthPercentage(
        currentOrders,
        previousOrders,
      );
      const usersGrowth = this.calculateGrowthPercentage(
        currentUsers,
        previousUsers,
      );
      ...
      return {
        ...
        newUsers: currentUsers, // Using same value for now
        averageOrderValue: currentAOV,
        averageOrderValueGrowth: aovGrowth,
        conversionRate: currentConversion,
        conversionRateGrowth: 0, // Default growth for conversion rate
        activeMerchants,
        topProducts,
        topMerchants,
        revenueChart,
      ...
      this.logger.error(
        `Failed to get analytics overview: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  private async calculateActiveMerchants(
    startDate: Date,
    endDate: Date,
  ): Promise<number> {
  ...
  private async calculateNewMerchants(
    startDate: Date,
    endDate: Date,
  ): Promise<number> {
  ...
  private async calculateTotalProducts(
    startDate: Date,
    endDate: Date,
  ): Promise<number> {
  ...
  private async calculateActiveProducts(
    startDate: Date,
    endDate: Date,
  ): Promise<number> {
    return this.prisma.product.count({
  ...
  private async calculateProductViews(
    startDate: Date,
    endDate: Date,
  ): Promise<number> {
  ...
  private async calculateAddToCartEvents(
    startDate: Date,
    endDate: Date,
  ): Promise<number> {
    return this.prisma.analyticsEvent.count({
  ...
  private async getTopProductsByRevenue(
    startDate: Date,
    endDate: Date,
    limit: number,
  ) {
  ...
  private async getTopMerchantsByRevenue(
    startDate: Date,
    endDate: Date,
    limit: number,
  ) {
