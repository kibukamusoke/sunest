...
export class ReportsService {
    ...
    try {
      ...
      switch (report.type) {
        ...
        case ReportType.MERCHANT_REPORT:
          reportData = await this.generateMerchantReport(
            startDate,
            endDate,
            parameters,
          );
        ...
        case ReportType.OPERATIONAL_REPORT:
          reportData = await this.generateOperationalReport(
            startDate,
            endDate,
            parameters,
          );
    ...
    const task = cron.schedule(
      ...
      async () => {
        try {
          ...
          this.logger.error(
            `Scheduled report execution failed: ${error.message}`,
            error.stack,
          );
        }
      },
  ...
  private async generateMerchantReport(
    startDate: Date,
    endDate: Date,
    parameters: any,
  ): Promise<any> {
  ...
  private async generateExecutiveSummary(
    ...
    return {
      ...
      growth: {
        ...
        customers: overview.revenueGrowth, // Using revenue growth as fallback
      },
      highlights: [],
      recommendations: [],
    };
  ...
  private async sendReportToRecipients(
    ...
    this.logger.log(
      `Would send report ${report.name} to ${recipients.join(', ')}`,
    );
  }
